...
std::vector<std::byte*> get_freelist_contents(const dd99::memory::structure::basic_linked_list & freelist) {
...
// -----------------------------------------------------------------------
bool get_bit(const dd99::memory::structure::Bitmap<std::byte> & bitmap, std::size_t index) {
    return bitmap[index];
}

std::size_t bitmap_size(const dd99::memory::structure::Bitmap<std::byte> & bitmap) {
